...
void Shader::CompileProgram()
{
	...
	unifromTexture = glGetUniformLocation(shaderID, "theTexture");
...
void Shader::SetSpotLights(SpotLight* sLight, unsigned int lightCount, unsigned int textureUnit, unsigned int offset)
{
	...
	for (size_t i = 0; i < lightCount; i++)
	{
		sLight[i].UseLight(uniformSpotLight[i].uniformAmbientIntensity, uniformSpotLight[i].uniformColour,
			...
			uniformSpotLight[i].uniformConstant, uniformSpotLight[i].uniformLinear, uniformSpotLight[i].uniformDirection,
...
{
	glUniform1i(unifromTexture, textureUnit);
}
...
void Shader::AddShader(GLuint theProgram, const char* shaderCode, GLenum shaderType)
{
	...
	if (!result)
	{
		...
		fprintf(stderr, "Error compiling the %d shader: \n%s\n", shaderType, eLog);
		return;
	}
